...
public class BinaryHeapForNodes {
   ...
   private void heapifyDown(int index) {
      ...
      if (compareByValue) {
         if (heapTable[leftChildIndex].compareTo1(heapTable[rightChildIndex]) == 1) {
         ...
         if (heapTable[leftChildIndex].compareTo(heapTable[rightChildIndex]) == 1) {
            biggestChild = rightChildIndex;
         }
   ...
   public String read() {
      ...
      for (Node n : heapTable) {
         sb.append(n.getSize()).append(" ").append(n.getValue()).append(" ").append((float) n.getSize() / (float) n.getValue()).append("\n");
      }
      return sb.toString();
   }
}
